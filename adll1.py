...
# ==========================================================
# PREMIUM CSS (dengan tambahan untuk Recommendations)
# ==========================================================
bg_style = f"""
.stApp {{
    background:
...
<style>
    ...
    /* ANIME CARD (Explorer & Recommendations) */
    .anime-card {{
        background: rgba(255,255,255,0.04);
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-section-title">Dataset Update</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="padding: 0 14px; font-size: 13px; color: #94a3b8;">
        May 19, 2024<br>
        <span style="color: #22c55e; font-size: 12px;">● Up to date</span>
    </div>
    """, unsafe_allow_html=True)
...
# ==========================================================
# PAGE: OVERVIEW (sama seperti sebelumnya)
# ==========================================================
...
# ==========================================================
# PAGE: ANALYTICS (sama seperti sebelumnya)
# ==========================================================
    ...
    st.dataframe(top_anime_table, use_container_width=True)
...
# ==========================================================
# PAGE: ANIME EXPLORER (sama seperti sebelumnya)
# ==========================================================
    ...
    if not page_data.empty:
        ...
        for idx, (_, row) in enumerate(page_data.iterrows()):
            with cols[idx % 3]:
                ...
                if img and pd.notna(img):
                    st.image(img, use_container_width=True, output_format="auto")
                else:
                    score = row["Score"]
                    if score >= 8:
...
# ==========================================================
# PAGE: USER ANALYTICS (sama seperti sebelumnya)
# ==========================================================
    ...
    st.dataframe(user_stats[["User", "Anime Watched", "Reviews", "Avg Score"]], use_container_width=True, hide_index=False)

# ==========================================================
# PAGE: RECOMMENDATIONS — PREMIUM (sesuai mockup)
# ==========================================================
    ...
    # --- Layout: 2 kolom (Filter + How it works) ---
    ...
    if selected:
        ...
        if anime_id in similarity_df.index:
            ...
            top_ids = sim_scores.iloc[1:11].index  # ambil 10 rekomendasi
            ...
            # Filter by selected genres (jika ada)
            ...
            # Tampilkan 10 rekomendasi (atau kurang jika difilter)
            st.markdown(f"""
            ...
            if not recs.empty:
                ...
                st.dataframe(recs[["Name", "Genres", "Score", "Type", "Episodes", "Similarity"]], use_container_width=True)
